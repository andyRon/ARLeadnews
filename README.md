...
Nacos（Dynamic Naming and Configuration Service）是阿里巴巴开源的服务发现和配置管理平台，支持：
...
arleadnews								父工程，统一管理项目依赖（定义通用包的版本），springboot
		...
		leadnews-service			管理一系列微服务
				leadnews-user				【51801】  用户端
        leadnews-article		【51802】  文章
        leadnews-wemedia		【51803】  自媒体
        leadnews-schedule		【51701】  延迟、定时任务
        leadnews-search			【51804】  搜索
		leadnews-test					测试案例，包含了多个技术验证 Demo
				es-init: Elasticsearch 索引初始化
        freemarker-demo: 模板引擎测试
        minio-demo: MinIO 对象存储集成
        kafka-demo: Kafka 消息队列示例
        mongo-demo: MongoDB 集成
        xxljob-demo: XXL-JOB 定时任务
        tess4j-demo: Tess4J OCR 文字识别
```
...
- 用户点击**开始使用**，登录后的用户权限较大，可以查看，也可以操作（点赞，关注，评论）。

- 用户点击**不登录，先看看**，游客只有查看的权限。
...
│                  Spring 应用上下文                           │
│                                                              │
│  最终生效的配置 = bootstrap.yml + Nacos 配置                 │
│  (Nacos 配置优先级更高，会覆盖本地相同配置)                   │
└─────────────────────────────────────────────────────────────┘
```
...
```
1. bootstrap.yml (最基础配置)
   ↓
2. application.yml (如果有的话)
   ↓
3. Nacos 远程配置 (优先级最高，会覆盖本地相同配置)
   ↓
4. 环境变量
   ↓
5. JVM 参数
...
###### 为什么需要分离bootstrap.yml和application.yml？
...
│  T0: 应用启动                                          │
│       ↓                                                 │
│  T1: 加载 bootstrap.yml (引导配置)                      │
│       ↓                                                 │
│  T2: 根据 bootstrap 配置连接 Nacos                       │
│       ↓                                                 │
│  T3: 从 Nacos 下载远程配置                               │
│       ↓                                                 │
│  T4: 加载 application.yml (应用配置)                    │
│       ↓                                                 │
│  T5: 合并所有配置 (Nacos 配置优先级高)                   │
│       ↓                                                 │
│  T6: 初始化 Spring 容器                                   │
│       ↓                                                 │
│  T7: 应用启动完成                                      │
...
| 文件                | 职责     | 典型配置内容                     |
| ------------------- | -------- | -------------------------------- |
| **bootstrap.yml**   | 引导配置 | Nacos 地址、应用名称、加密密钥等 |
| **application.yml** | 应用配置 | 业务配置、Bean 定义、日志级别等  |
...
传统方式：
  ...
  service-2/src/main/resources/application.yml
  service-3/src/main/resources/application.yml
  ... (分散在各个项目中，难以统一管理)

Nacos 方式：
  Nacos Config Center
  ├── leadnews-article.yml
  ├── leadnews-user.yml
  ├── leadnews-wemedia.yml
  └── ... (所有配置集中管理)
```
...
logging:
  level:
    top.andyron.article: DEBUG  # 从 INFO 改为 DEBUG
```
...
│ 版本   │ 发布时间       │ 操作人     │
├──────────────────────────────────────┤
│ V10    │ 2024-03-16 10:00 │ admin   │
│ V9     │ 2024-03-15 15:30 │ admin   │
│ V8     │ 2024-03-14 09:20 │ developer│
│ ...    │ ...            │ ...       │
└──────────────────────────────────────┘
...
```
专属配置 (leadnews-article.yml)
    ↑ (优先级最高，会覆盖共享配置的相同项)
共享配置 (common-datasource.yml)
    ↑
扩展配置 (common-kafka.yml)
    ↑ (优先级最低)
```

###### 总结

**`bootstrap.yml`与Nacos配置的关系**：

| 对比项       | bootstrap.yml        | Nacos 配置                  |
| ------------ | -------------------- | --------------------------- |
| **位置**     | 本地项目资源目录     | Nacos 配置中心服务器        |
| **加载时机** | 应用启动时最先加载   | 根据 bootstrap 配置随后加载 |
| **作用**     | 指导如何获取远程配置 | 提供实际的业务配置          |
| **更新方式** | 需要重新打包部署     | Nacos 控制台动态修改        |
| **优先级**   | 较低                 | 较高（覆盖本地相同配置）    |
| **典型内容** | Nacos 地址、应用名称 | 数据库、Redis、Kafka 等配置 |
...
### 1.5 接口工具postman、swagger、knife4j
...
knife4j是为Java MVC框架集成Swagger生成Api文档的增强解决方案,前身是swagger-bootstrap-ui,取名kni4j是希望它能像一把匕首一样小巧,轻量,并且功能强悍! 【swagger的增加版】
...
按照性能和收费来看，AR头条项目使用的就是阿里云的内容安全接口，使用到了图片和文本的审核。
...
```java
    ...
    @Test
    public void testDel(){
        mongoTemplate.remove(Query.query(Criteria.where("associateWords").is("AR头条")),ApAssociateWords.class);
    }
```
...
AR头条项目整个项目开发涉及web展示和大数据分析来给用户推荐文章，如何找出哪些文章是热点文章进行针对性的推荐呢？这个时候需要进行大数据分析的准备工作，埋点。
